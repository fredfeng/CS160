...
let expect_generic msg expected actual =
  if expected <> actual then
    raise
      (TypeError
         (Printf.sprintf "%s: expected type: %s, actual type: %s" msg
            (expected |> Ast.show_typ) (actual |> Ast.show_typ)))
  else ()
...
let assert_eq t1 t2 s1 s2 =
  if t1 <> t2 then
    raise
      (TypeError
         (Printf.sprintf "mismatch between type of %s (%s) and type of %s (%s)"
            s1 (t1 |> Ast.show_typ) s2 (t2 |> Ast.show_typ)))
  else ()
...
let unbound_var x = semantic_error ("Unbound variable " ^ x)
let unbound_fn f = semantic_error ("Unbound function " ^ f)
let main_called () = semantic_error "Function main should not be called"
let no_main () = semantic_error "Please provide a main function"
let main_param () = semantic_error "Function main should not have any parameter"
let main_return () = semantic_error "Function main should return unit"
let empty_seq () = semantic_error "Empty sequence"
let duplicated_fn f = semantic_error ("Duplicated function definition: " ^ f)

let duplicated_param f x =
  semantic_error (Printf.sprintf "Duplicated parameter %s in function %s" x f)

let arg_length_mismatch f =
  semantic_error (Printf.sprintf "Wrong number of arguments to function %s" f)
